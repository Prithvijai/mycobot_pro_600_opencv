...
calibration_file = '/path/to/your/calibration_file.yaml'

with open(calibration_file, 'r') as file:
    calibration_data = yaml.safe_load(file)

# Extract camera matrix and distortion coefficients from the YAML data
camera_matrix = np.array(calibration_data['K']).reshape((3, 3))  # Reshape to 3x3
dist_coeffs = np.array(calibration_data['D'])  # Assuming D is already a 1x5 array
